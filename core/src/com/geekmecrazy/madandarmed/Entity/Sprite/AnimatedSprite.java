...
public class AnimatedSprite extends Sprite {
	...
	public void setCurrentFrame(final int pCurrentFrameX, final int pCurrentFrameY) {
		...
		if(this.getSpriteSheet().isFromTexturePack()){
			this.setOffsetX(this.getCurrentFrameOffsetX()+(this.getTextureRegion().getRegionWidth()/2f)-(this.getCurrentFrameOriginalWidth()/2f));
			this.setOffsetY(this.getCurrentFrameOffsetY()+(this.getTextureRegion().getRegionHeight()/2f)-(this.getCurrentFrameOriginalHeight()/2f));
		}
	}
	...
	public float getCurrentFrameOriginalWidth(){
		return this.getSpriteSheet().getFrameOriginalWidth(this.getCurrentFrameX(), this.getCurrentFrameY());
	}

	public float getCurrentFrameOriginalHeight(){
		return this.getSpriteSheet().getFrameOriginalHeight(this.getCurrentFrameX(), this.getCurrentFrameY());
	}
